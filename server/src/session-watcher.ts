...
function initOffset(eventsPath: string) {
  try {
    ...
    console.warn('[SessionWatcher] Failed to init offset for:', eventsPath, err);
  }
}
...
export function watchSessionEvents(callback: MessageCallback) {
  ...
  try {
    ...
    console.warn('[SessionWatcher] Failed to read session state directory during init:', err);
  }
